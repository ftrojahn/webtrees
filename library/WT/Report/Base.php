...
class WT_Report_Base {
	...
	function setup() {
		...
		$this->generatedby = WT_I18N::translate('Generated by %s', WT_WEBTREES);
...
function AgeAtDeathSHandler() {
	...
	if (preg_match("/0 @(.+)@/", $gedrec, $match)) {
		...
		if (WT_Date::Compare($birth_date, $death_date) <= 0 || !$person->isDead()) {
			...
			if ($age != '' && $age != "0d") {
				if ($fact_age != '' && $fact_age != $age || $fact_age == '' && $husb_age == '' && $wife_age == '' || $husb_age != '' && $person->getSex(
					) == 'M' && $husb_age != $age || $wife_age != '' && $person->getSex() == 'F' && $wife_age != $age
				) {
